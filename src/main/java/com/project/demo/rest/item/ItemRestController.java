...
public class ItemRestController {
    ...
    public ResponseEntity<?> getAll(
        ...
        meta.setTotalPages(itemsPage.getTotalPages());
        meta.setTotalElements(itemsPage.getTotalElements());
        meta.setPageNumber(itemsPage.getNumber() + 1);
        meta.setPageSize(itemsPage.getSize());

        return new GlobalResponseHandler().handleResponse("Order retrieved successfully",
                itemsPage.getContent(), HttpStatus.OK, meta);
    }
    ...
    public ResponseEntity<?> patchItem(@PathVariable Long itemId, @RequestBody Item item, HttpServletRequest request) {
        ...
        if(foundItem.isPresent()) {
            ...
            if(item.getPrice() != null) foundItem.get().setPrice(item.getPrice());
            if(item.getStatus() != null) foundItem.get().setStatus(item.getStatus());
            itemRepository.save(foundItem.get());
            return new GlobalResponseHandler().handleResponse("Item updated successfully",
